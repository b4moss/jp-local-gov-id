...
Prefer loading `index.json` + `.bin` files via `url` rather than bundling `dataset.js`.
...
2. Place `index.json`, `prefectures.bin`, and `prefectures/` (containing `.bin` files) together (you do not need `dataset.js`)
