...
import { isMunicipality, isPrefecture } from "./types";
 ...
 * Prefecture records are never filtered.
 */
export function filterByDesignatedCity<T extends LocalGov>(
  items: readonly T[],
  mode: DesignatedCityMode = "both",
): T[] {
  if (mode === "both") return [...items];

  const municipalities = items.filter(isMunicipality);
  ...
  return items.filter((item) => {
    if (isPrefecture(item)) return true;
